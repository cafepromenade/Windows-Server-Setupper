...
function New-Fixture([string]$Name) {
    ...
    [IO.File]::WriteAllBytes((Join-Path $squirrel 'ExchangeAutoInstaller-1.2.3-x64-Setup.exe'), [byte[]]::new(131072))
    ...
    [IO.File]::WriteAllBytes((Join-Path $unpacked 'Exchange Auto Installer.exe'), [byte[]]::new(131072))
...
function Expect-Failure([string]$Name, [scriptblock]$Mutate, [string]$ExpectedMessage) {
    ...
    try {
        & (Join-Path $PSScriptRoot 'verify-exchange-package.ps1') -SourceCommit $commit -OutputRoot $relative *> $null
...
try {
    ...
    Expect-Failure 'corrupt-setup-pe' { param($dist) } 'does not have an MZ header'
}
finally {
    if (Test-Path -LiteralPath $fixtureRoot) { Remove-Item -LiteralPath $fixtureRoot -Recurse -Force -ErrorAction SilentlyContinue }
}

Write-Output 'PASS: 11 missing/corrupt release-asset fixtures turned red'
