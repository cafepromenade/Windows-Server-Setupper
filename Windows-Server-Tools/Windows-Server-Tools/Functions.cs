...
if (-not (Get-DhcpServerInDC -DnsName $computerName -ErrorAction SilentlyContinue)) {
    Add-DhcpServerInDC -DnsName $computerName -IPAddress $address;
        ...
        private static string GetTrustedCommonApplicationDataDirectory()
        {
            ...
            ValidateTrustedDirectoryPathComponents(commonApplicationData);
            ...
            string commonApplicationData)
        {
            ...
            FileAttributes chocolateyRootAttributes;
            bool chocolateyRootExists = TryGetExistingFileSystemAttributes(
                chocolateyRoot,
                out chocolateyRootAttributes);
            if (chocolateyRootExists
                && (chocolateyRootAttributes & FileAttributes.Directory) == 0)
            {
                throw new InvalidOperationException(
            ...
            if (chocolateyRootExists
                && (chocolateyRootAttributes & FileAttributes.ReparsePoint) != 0)
            {
                throw new InvalidOperationException(
                    "The Chocolatey installation directory cannot be a reparse point.");
            }
            ...
            string executablePath)
        {
            ...
            FileAttributes binAttributes;
            bool binExists = TryGetExistingFileSystemAttributes(
                binPath,
                out binAttributes);
            if (binExists && (binAttributes & FileAttributes.Directory) == 0)
            {
                throw new InvalidOperationException(
                    "The Chocolatey bin directory is occupied by a file. Remove it only after an administrator verifies that it is safe.");
            }

            if (binExists && (binAttributes & FileAttributes.ReparsePoint) != 0)
            {
                throw new InvalidOperationException(
                    "The Chocolatey bin directory cannot be a reparse point.");
            }
            ...
            bool executableExists = TryGetExistingFileSystemAttributes(
                executable,
                out executableAttributes);
            if (executableExists
            ...
            {
                throw new InvalidOperationException(
                    "The Chocolatey executable cannot be a reparse point.");
            }

            if (executableExists)
            {
                ValidateTrustedChocolateyFile(executable);
            }
        }

        private static bool TryGetExistingFileSystemAttributes(
            string path,
            out FileAttributes attributes)
        {
            try
            {
                attributes = File.GetAttributes(Path.GetFullPath(path));
                return true;
            }
            ...
            bool sharedCommonApplicationDataRoot)
        {
            ...
            {
                throw new InvalidOperationException(
                    "A Chocolatey installation path has an owner other than Administrators, SYSTEM, or TrustedInstaller.");
            }
        ...
        private static void ValidateTrustedDirectoryPathComponents(string directoryPath)
        {
            var components = new Stack<DirectoryInfo>();
            ...
            while (components.Count > 0)
            {
                ...
                ValidateTrustedPathComponentAcl(
                    current.GetAccessControl(
                        AccessControlSections.Access | AccessControlSections.Owner));
            }
        }

        private static void ValidateTrustedPathComponentAcl(FileSystemSecurity security)
        {
            var system = new SecurityIdentifier(WellKnownSidType.LocalSystemSid, null);
            var administrators = new SecurityIdentifier(
                WellKnownSidType.BuiltinAdministratorsSid,
            ...
            {
                throw new InvalidOperationException(
                    "A per-machine application-data path component has an untrusted owner.");
            }

            FileSystemRights unsafeRights = FileSystemRights.Delete
                | FileSystemRights.DeleteSubdirectoriesAndFiles
                | FileSystemRights.ChangePermissions
                | FileSystemRights.TakeOwnership;
            foreach (FileSystemAccessRule rule in security.GetAccessRules(
                ...
                typeof(SecurityIdentifier)))
            {
                ...
                {
                    throw new InvalidOperationException(
                        "A per-machine application-data path component grants an untrusted identity delete or permission-control rights.");
                }
            }
        }
        ...
        private static FileStream OpenVerifiedFileForRead(
            ...
            string expectedSha256)
        {
            ValidateTrustedChocolateyFile(filePath);
            ...
            string expectedSha256)
        {
            if (string.IsNullOrWhiteSpace(expectedSha256))
            {
                throw new ArgumentException(
                    "An expected Chocolatey executable SHA-256 digest is required.",
            ...
            ValidateExistingChocolateyInstallParents(
                ...
                chocolateyRoot,
                executable);
            using (FileStream executableLock = OpenVerifiedFileForRead(
                executable,
                expectedSha256))
            {
                ValidateExistingChocolateyInstallParents(
