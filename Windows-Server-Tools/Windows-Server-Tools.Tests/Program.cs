...
namespace Windows_Server_Tools.Tests
{
    internal static class Program
    {
        ...
        private static async Task Run()
        {
            ...
            GlobalLeaseNameIsStableAcrossSessions();
        ...
        private static void GlobalLeaseNameIsStableAcrossSessions()
        {
            string directory = NewTemporaryDirectory();
            string stateFile = Path.Combine(directory, "global.steps");
            string first = BatchFileLease.GetMutexNameForTest(stateFile);
            string second = BatchFileLease.GetMutexNameForTest(Path.Combine(directory, ".", "global.steps"));
            Check(first.StartsWith("Global\\WindowsServerToolsRecovery_", StringComparison.Ordinal)
                && string.Equals(first, second, StringComparison.Ordinal),
                "the same recovery state path should map to one Global namespace lease across interactive sessions");
        }
