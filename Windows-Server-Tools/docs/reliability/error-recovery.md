...
A named cross-process mutex derived from the normalized checkpoint path serializes each batch. The acquisition is bounded; a second process does not wait indefinitely or merge state without owning the lease.
...
2. copy every recovery candidate and marker into a uniquely named archive directory;
3. hash and re-read each archived item and write a manifest;
...
Each process is assigned to a Windows Job Object configured with kill-on-close behavior. Completion waits for both the initial process and the Job Object's active process count to reach zero, so a launcher process cannot report success while a child continues in the background.
...
If Job Object assignment itself fails, the application attempts to terminate the uncontained process and reports an indeterminate failure rather than continuing without containment.
...
Both the package and extracted installer are hashed. Verified read handles remain open while the installer starts, and the installed executable path and reported version are checked afterward. On failure, bounded evidence remains in the protected attempt directory. Successful installation removes only the attempt directory whose ownership marker matches the current attempt.

The server-role launch controls that lead to the legacy Exchange/SCCM delivery chain—including **Promote to Omega Server**, **SCCM Only**, and **Install Side Server**—remain disabled. Their visible and accessibility text explains that no download or launch will occur until credential-safe replacement artifacts are rebuilt, published, and pinned.
...
| Another recovery batch is active | The bounded cross-process lease was unavailable. | Let the owning batch finish, then retry. Do not delete the checkpoint. |
...
- The legacy Exchange/SCCM delivery artifacts are not considered credential-safe and remain unreachable from the hardened primary UI.
...
The focused executable links the production `Recovery.cs` into a small .NET Framework 4.7.2 test project. On the current local source, the exact commands were:
...
This is the latest completed local baseline. Source review continued afterward, so the executable must be rerun after the final source freeze; the final integrated count may increase when new checks are added.
...
That baseline scratch build succeeded and produced `Windows-Server-Tools.exe`. SHA-256 comparison confirmed that the copied `Recovery.cs`, WPF XAML/code-behind, `Functions.cs`, and `App.xaml.cs` matched the repository files at the time of the build. Source review continued afterward; a final source-hash comparison and scratch rebuild remain required once the source is frozen. The scratch project and downloaded reference assemblies are test-environment material outside the repository; they are evidence for that compile only, not a shipped build path.
