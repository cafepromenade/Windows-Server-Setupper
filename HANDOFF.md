...
| Deliberately broken workflow contract | `8/8` mutations turned red |
...
`scripts/validate-release-contract.ps1 -SelfTest` proves eight independent workflow/inventory regressions turn red, including missing triggers, wrong runner, missing root installer route, a prohibited test command, missing always-on failure collection, a mutable action tag, missing release-download proof, and missing Exchange package verification.
